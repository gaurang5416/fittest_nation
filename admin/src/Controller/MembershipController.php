...
use spec\Umpirsky\Twig\Extension\PhpFunctionExtensionSpec;
...
Class MembershipController extends AppController
{
    ...
    public function add()
    {
        ...
        $installment_plan->select(['id',
            'concatenated' => $installment_plan->func()->concat(['number' => 'literal', ' ', 'duration' => 'literal'])
        ...
        if ($this->request->is("post")) {
            ...
            if (count($_FILES['gmgt_membershipimage']['name']) > 0 && $_FILES['gmgt_membershipimage']['name'][0] != "") {
                foreach ($this->request->data["gmgt_membershipimage"] as $var) {
                    $ext = $this->GYMFunction->check_valid_extension($var['name'], 'image_video');
                    if ($ext != 0) {
                        $image_name = $this->GYMFunction->uploadImage($var);
                        $image_array[] = $image_name;
                    }
                }
                $this->request->data["gmgt_membershipimage"] = json_encode($image_array);
            }
    ...
    public function editMembership($id)
    {
        ...
        if ($this->request->is("post")) {
            ...
            $row_image_array = $row['gmgt_membershipimage'] !== '' && $row['gmgt_membershipimage'] !== null ? json_decode($row['gmgt_membershipimage']) : [];
            if (isset($this->request->data['removed_images']) && $this->request->data['removed_images'] != null) {
                ...
                $row_image_array = array_diff($row_image_array, $images);
            ...
            if (count($_FILES['gmgt_membershipimage']['name']) > 0 && $_FILES['gmgt_membershipimage']['name'][0] != "") {
                foreach ($this->request->data["gmgt_membershipimage"] as $var) {
                    ...
                    if ($ext != 0) {
                        $image_name = $this->GYMFunction->uploadImage($var);
                        $row_image_array[] = $image_name;
                    }
                }
            }
            $this->request->data["gmgt_membershipimage"] = count($row_image_array) > 0 ? json_encode($row_image_array) : '';
